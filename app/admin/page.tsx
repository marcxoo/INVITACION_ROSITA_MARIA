...
export default function AdminDashboard() {
    ...
    async function fetchGuests() {
        ...
        const { data, error } = await supabase
            .from('invitations')
            .select('*')
            .eq('event_slug', 'rosita-maria') // Filter for Rosita
    ...
    async function handleDeleteAll() {
        ...
        if (!confirm("¿Estás seguro de borrar SOLO los datos de Rosita?")) return;

        // Delete only Rosita's records
        await supabase
            .from('invitations')
            .delete()
            .eq('event_slug', 'rosita-maria');
    ...
    return (
        <div className="min-h-screen flex flex-col items-center p-6 bg-[#FDFBF7] relative overflow-x-hidden"
            ...
            <div className="relative z-10 text-center mb-10 mt-4">
                <h1 className="text-5xl md:text-7xl font-bold text-[#7A2D3E] font-vibes drop-shadow-sm">Rosita María</h1>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-[#7A2D3E]/10">
                            {guests.length === 0 ? (
                                <tr><td colSpan={3} className="p-10 text-center text-gray-400 italic">No hay registros para 'Rosita María' aún...</td></tr>
                            ) : (
                                guests.map((g, i) => {
